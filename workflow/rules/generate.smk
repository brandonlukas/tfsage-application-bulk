...
rule synthesize_experiments:
    input:
        distances=config["results_dir"] + "data/{threshold}/distances/{mode}.parquet",
...
rule generate_test_set:
    ...
    output:
        config["results_dir"]
        + "generate/exclude_holdout/{threshold}/test_sets/{test_set_name}/{query_id}_{target_id}.parquet",
    params:
...
rule compute_metrics_tfsage:
    input:
        ...
        + "generate/exclude_holdout/{threshold}/tfsage_experiments/head_{n}/{query_id}_{factor}.parquet",
        test_set=config["results_dir"]
        + "generate/exclude_holdout/{threshold}/test_sets/{test_set_name}/{query_id}_{target_id}.parquet",
    output:
        config["results_dir"]
        + "generate/exclude_holdout/{threshold}/metrics/{test_set_name}/tfsage/head_{n}/{query_id}_{factor}_{target_id}.json",
    params:
        method_class="tfsage",
...
rule compute_metrics_motif_scan:
    ...
    output:
        config["results_dir"]
        + "generate/exclude_holdout/{threshold}/metrics/{test_set_name}/motif_scan/{motif_db}/{query_id}_{factor}_{target_id}.json",
    params:
